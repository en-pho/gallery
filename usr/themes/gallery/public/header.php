<header class="joe_header <?php echo $this->is('post') ? 'current' : '' ?>">
      ...
      <nav class="joe_header__above-nav">
        ...
        <?php if (count($pages->stack) <= $this->options->JNavMaxNum) : ?>
          <?php foreach ($pages->stack as $item) : ?>
            <a class="item <?php echo $this->is('page', isset($item['slug']) ? $item['slug'] : '') ? 'active' : '' ?>" href="<?php echo isset($item['permalink']) ? $item['permalink'] : '#' ?>" title="<?php echo isset($item['title']) ? $item['title'] : '' ?>"><?php echo isset($item['title']) ? $item['title'] : '' ?></a>
          <?php endforeach; ?>
        <?php else : ?>
          ...
          <div class="joe_dropdown" trigger="hover" placement="60px" style="margin-right: 15px;">
            ...
            <nav class="joe_dropdown__menu">
              <?php foreach (array_slice($pages->stack, $this->options->JNavMaxNum) as $item) : ?>
                <a class="<?php echo $this->is('page', isset($item['slug']) ? $item['slug'] : '') ? 'active' : '' ?>" href="<?php echo isset($item['permalink']) ? $item['permalink'] : '#' ?>" title="<?php echo isset($item['title']) ? $item['title'] : '' ?>"><?php echo isset($item['title']) ? $item['title'] : '' ?></a>
              <?php endforeach; ?>
            </nav>
          </div>
          <?php endif; ?>
  ...
  <div class="joe_header__slideout">
    ...
    <ul class="joe_header__slideout-count">
      <?php Snowfox_Widget::widget('Widget_Stat')->to($count); ?>
      <li class="item">
        <svg class="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="15" height="15">
