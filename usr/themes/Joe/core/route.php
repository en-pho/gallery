...
function _getArticleFiling($self)
{
    ...
    foreach ($temp as $item) {
        ...
        foreach ($_list as $_item) {
            ...
            $_item['category'] = urlencode(current(Snowfox_Common::arrayFlatten($_item['categories'], 'slug')));
            $_item['slug'] = urlencode($_item['slug']);
            $_item['date'] = new Snowfox_Date($_item['created']);
            $_item['year'] = $_item['date']->year;
            $_item['month'] = $_item['date']->month;
            $_item['day'] = $_item['date']->day;
            $routeExists = (NULL != Snowfox_Router::get($type));
            $_item['pathinfo'] = $routeExists ? Snowfox_Router::url($type, $_item) : '#';
            $_item['permalink'] = Snowfox_Common::url($_item['pathinfo'], $options->index);
            $list[] = array(
